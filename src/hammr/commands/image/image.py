        ...
        try:
            ...
            if images is None or len(images) == 0:
                ...
                for image in images:
                    ...
                    table.add_row([image.dbId, image.name, image.version, image.revision, image.targetFormat.name, image.created.strftime("%Y-%m-%d %H:%M:%S"), size(image.size), "X" if image.compress else "", imgStatus])
                print table.draw() + "\n"
